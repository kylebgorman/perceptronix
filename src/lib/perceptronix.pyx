...
class PerceptronixError(Exception):

  pass


class PerceptronixIOError(IOError, PerceptronixError):

  pass


class PerceptronixOperationError(RuntimeError, PerceptronixError):
...
cdef class DenseBinomialClassifier(object):
  ...
  Binomial linear classifier backed by a fixed-size contiguous array of weights.
  ...
  cpdef void average(self) except *:
    ...
    * Prediction with `predict` becomes faster and should generalizes better.
    * Serialization with (`write`) becomes possible.
    * The memory footprint of the model shrinks to approximately one third of
      pre-averaged model.
    ...
    This method cannot be invoked on an instance created by deserialization with
    `read`, as such models are already averaged.
    """
    if self._averaged():
  ...
  cpdef bool train(self, features, bool label) except *:
    ...
    Returns:
      A boolean indicating whether the instance as already correctly labeled;
          this can be used to compute a epoch's resubstitution accuracy.
...
cdef class SparseBinomialClassifier(object):
  ...
  table; unlike DenseBinomialClassifier, though, this is not a hard constraint.
  During training and inference, users pass features as iterables of strings.
  """
...
cdef class DenseMultinomialClassifier(object):
  ...
  construction time, clients specify the maximum number of unique features
  and labels. During training and inference, users pass features as iterables of
  non-negative integers and labels as non-negative integers.
  """
...
cdef class SparseDenseMultinomialClassifier(object):
  ...
  Like SparseMultinomialClassifier, this is not a hard constraint. The latter is
  used to compute the max size for the inner array of weights; like
...
cdef class SparseMultinomialClassifier(object):
  ...
  estimated number of features and labels, which is used to compute the initial
  sizes for the nested hash tables; unlike DenseBinomialClassifier, neither are
  hard constraints. During training and inference, users pass features as
  iterables of strings and labels as strings.
  """
